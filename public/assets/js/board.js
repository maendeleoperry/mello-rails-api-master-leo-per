 ...
 Sortable.create($boardContainer[0], {
  ...
  onMove: function(event) {
    ...
    onEnd: function(event) {
      let { id, position } = $(event.item).date();
...
function handleContributorSave(event) {
  ...
  let contributorEmail = $contributorModalInput.val().trim();
    ...
    if (contributor) {
      displayMessage(
        `${contributorEmail} already has access to board`,
        'danger'
        );
      return;
    }
...
function handleContributeDelete() {
...
$contriborModalSaveButton.on('click', handleContributorSave);
